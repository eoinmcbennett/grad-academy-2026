...
- `new-part-1/0-setup/0-setup.md` - Terminal Setup (macOS): installing Chrome from Company Portal, downloading iTerm2, installing Oh My Zsh, setting a `ZSH_THEME`, installing the VS Code `code` CLI command in PATH, and validating the full terminal workflow (`npm run dev:terminal-setup`)
